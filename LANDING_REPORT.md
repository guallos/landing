...
| Catálogo con **menú de categorías fijo** que resalta la sección visible (en móvil, chips horizontales), íconos por servicio y una **vista previa flotante del documento** que sigue al cursor, con su fundamento legal, precio y sello | CSS + `main.js` (menú) + `fx.js` (vista previa) | El menú funciona sin efectos; la vista previa solo aparece con mouse y sin movimiento reducido |
